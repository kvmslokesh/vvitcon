...
const Post = ({ post }) => {
	...
	return (
		<>
			<div className='flex gap-4 items-start p-4 border-b border-gray-300 my-4 neumorphic2'>
				...
				<div className='flex flex-col flex-1'>
					...
					<div className='flex justify-between mt-3'>
						<div className='flex gap-4 items-center w-2/3 justify-between'>
							...
							<dialog id={`comments_modal${post._id}`} className='modal border-none outline-none'>
								<div className='modal-box rounded border border-gray-300'>
									...
									>
										<textarea
											className='textarea w-full p-1 rounded text-md resize-none border focus:outline-none border-gray-300'
							...
							<div className='flex gap-1 items-center group cursor-pointer'>
								<BiRepost className='w-6 h-6 text-gray-500 group-hover:text-green-500' />
								<span className='text-sm text-gray-500 group-hover:text-green-500'>0</span>
							</div>
							<div className='flex gap-1 items-center group cursor-pointer' onClick={handleLikePost}>
								{isLiking && <LoadingSpinner size='sm' />}
								{!isLiked && !isLiking && (
									<FaRegHeart className='w-5 h-5 cursor-pointer text-gray-500 group-hover:text-red-500' />
								)}
								{isLiked && !isLiking && (
									<FaRegHeart className='w-5 h-5 cursor-pointer text-red-500' />
								)}
						...
						<div className='flex w-1/3 justify-end gap-2 items-center'>
							<FaRegBookmark className='w-5 h-5 text-gray-500 cursor-pointer' />
						</div>
					</div>
				</div>
			</div>
